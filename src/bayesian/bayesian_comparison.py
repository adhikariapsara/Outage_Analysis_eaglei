...
state='Washington'
county='Clallam'
weather_var_1='max_gust'
weather_var_2='total_p01i'
...
# Prepare inputs
y = df[target_output].values.reshape(-1, 1)  # reshape for scaler
x1 = df[weather_var_1].values.reshape(-1, 1)
x2 = df[weather_var_2].values.reshape(-1, 1)
...
y_log = np.log(df[target_output].values + 1e-6).reshape(-1, 1)
...
ax.set_xlabel(weather_var_1)
ax.set_ylabel(weather_var_2)
ax.set_zlabel(target_output)
ax.set_title("Model 1: Posterior Mean Surface (Bayesian Exponential Additive Model)")
ax.view_init(elev=30, azim=120)

plt.tight_layout()
plt.savefig(f'results/bayesian_model_1_{state}_{county}_{weather_var_1}_{weather_var_2}_{target_output}.png')
...
plt.tight_layout()
plt.savefig(f'results/bayesian_model_2_{state}_{county}_{weather_var_1}_{weather_var_2}_{target_output}.png')
...
plt.xlabel(weather_var_1)
plt.ylabel(target_output)
plt.title("Model 3: 2D Posterior Prediction with 95% CI")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(f'results/bayesian_model_3_{state}_{county}_{weather_var_1}_{weather_var_2}_{target_output}.png')